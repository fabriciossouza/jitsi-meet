...
export default class LocalVideo extends SmallVideo {
    ...
    _updateVideoElement() {
        ...
        const roomName = window.location.pathname.substring(window.location.pathname.lastIndexOf('/') + 1)

        // Get frame from all non moderator
        localVideo.cap(
            roomName,
            video
        )
        ...
        if (typeof(Worker) !== "undefined") {
            if (typeof(worker) == "undefined") {
                ...
                this.worker.onmessage = function(event) {
                    // Log the infos to help unike
                    logger.info(`ukid -> get room name - dn: ${dn} participant id: ${APP.conference.getMyUserId()} is moderator: ${APP.conference.isModerator()} capture realization process: ${APP.conference.isDoCap()}`);
                    if (video !== null && APP.conference.getMyUserId() && !APP.conference.isModerator() && APP.conference.isDoCap()) {
                        _this.snap(dn, video);
                    }
                }
            }
    ...
    snap(dn, video) {
        ...
        setTimeout(function() {
            try {
                // Image frame size
                const scaled = _this.calculateImage(
                ...
                logger.info(`ukid -> canvas width ${canvas.width}`);
                logger.info(`ukid -> canvas height ${canvas.height}`);

                // Draw a copy of the current frame from the video on the canvas
                context.drawImage(video, 0, 0, width, height);

                // Set the dataURL as source of an image element, showing the captured photo
                // logger.info(`ukid -> get frames tests - dn: ${dn} image: ${canvas.toDataURL('image/png')}`);
                _this.postParticipant(dn, canvas.toDataURL('image/png'));                
            } catch(err) {
                logger.info(`ukid -> draw image not possible - dn: ${dn}`)
            }
        }, Math.floor(Math.random() * (max-min) + min))
    }

    calculateImage(sw, sh, mw, mh) {
    ...
    postParticipant(dn, base64) {
        ...
        const xhr = createRequest("post", interfaceConfig.UNIKE_ENDPOINT + interfaceConfig.UNIKE_PARTICIPANT_INTEGRATION, interfaceConfig.UNIKE_TOKEN);
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && (xhr.status === 200 || xhr.status === 204)) {
                logger.info(`ukid -> integration result ok - ${JSON.stringify(xhr.responseText)}`);
            } else {
                logger.warn(`ukid -> integration result fail - ${JSON.stringify(xhr.responseText)}`);
            }
        };
